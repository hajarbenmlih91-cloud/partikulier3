...
* Tri des résidus sécurité du bruit PHPCS (SE-020, campagne post-audit — P1-6) : sanitiseur d'options par liste blanche clé→callback (E-2003, nouveau module `inc/class-options-sanitizer.php`) appliqué à `pk_opts` / `pk_theme_options` AVANT toute persistance — le transient de validation de l'écran de personnalisation ne stocke plus jamais la charge parasite (clés inconnues ignorées, `<script>` assaini) ; verbe HTTP assaini (`sanitize_key`, repli GET conforme RFC 3875, comparaison en minuscules — E-2004, module de cache, SAPI cli exclue) ; URL de retour du pont d'authentification Estatik validée `wp_validate_redirect` — une URL externe est désormais abandonnée, vecteur d'hameçonnage après connexion reproduit sur banc (E-2005) ; redirection de dépôt via `wp_safe_redirect` (E-2006) ; JSON-LD inline hex-échappé (`JSON_HEX_TAG|JSON_HEX_AMP` — une séquence `</script>` dans un titre importé brut ne peut plus rompre le contexte script HTML, preuve dynamique au contrat) ; titres `wp_die` échappés (`esc_html__`). Rejeu PHPCS `WordPress.Security.*` sur le périmètre runtime (124 fichiers) : 0 constat — ruleset `phpcs.xml.dist` à la racine du monorepo (exclusion justifiée E-2007 des lectures `$_GET` publiques sans nonce, sévérité ERROR `$_POST` toujours active), annotations ciblées avec justification, tri complet des 140 occurrences versionnées dans les preuves du kit de recette (16 réels corrigés / 57 faux positifs documentés / 67 risques acceptés documentés). Nouveau verrou : contrat `options-sanitizer-contract` (8 assertions OS-001→008, oracle 264/264).
...
* Reconciliation du dequeue Estatik (SE-018, campagne post-audit — P1-4) : les DEUX hooks publics d'Estatik 4.3.x sont retirés (register_global_assets, source mesurée du jQuery frontal, ET frontend_assets — le retrait d'origine n'en couvrait qu'un) ; le repli de dépendances ne peut plus ressusciter es-select2/es-datetime-picker (dependance jQuery) hors des parcours proprietaires ; le filet dequeue_heavy etendu (es-framework, es-frontend, es-properties, wp-color-picker) avec wp_deregister_script systematique ; promesse style.css reformulee (« zero jQuery sur les pages editoriales », exception proprietaire documentee DP-6) ; nouveau contrat front-assets (tests/front-assets.mjs, 7 assertions : pages editoriales sans jQuery + garde de version Estatik 4.3.x). NOTE de deploiement : purger le cache de pages integre a la mise a jour (les pages deja en cache servent le HTML d'avant-correctif).
