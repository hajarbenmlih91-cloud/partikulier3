...
class Partikulier_Customization {
	...
	public static function save() {
		if ( ! current_user_can( 'manage_options' ) ) {
			...
			/* SE-020 (E-2003) : sanitiseur liste blanche clé -> callback AVANT toute
				persistance (option OU transient de validation) — les clés inconnues
				et la charge parasite ressortent ignorées. */
			$posted = isset( $_POST['pk_opts'] ) && is_array( $_POST['pk_opts'] ) ? Partikulier_Options_Sanitizer::sanitize_customization( wp_unslash( $_POST['pk_opts'] ) ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wp_unslash + sanitize_customization (liste blanche E-2003)
			...
			$theme_posted = isset( $_POST['pk_theme_options'] ) && is_array( $_POST['pk_theme_options'] ) ? Partikulier_Options_Sanitizer::sanitize_customization( wp_unslash( $_POST['pk_theme_options'] ) ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wp_unslash + sanitize_customization (liste blanche E-2003)
