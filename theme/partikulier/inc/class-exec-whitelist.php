 ...
 *  - mode dégradé : binaire absent, empreinte invalide ou exec() désactivée
 *    => refus propre, jamais d'erreur fatale, état journalisé et visible via
 *    resolve()/is_available()/journal_path().
 *
...
class Partikulier_Exec_Whitelist {
	...
	/**
	 * Résolution d'une clef : premier candidat exécutable, empreinte contrôlée.
	 *
	 * États : 'ok' | 'absent' | 'fingerprint' | 'exec' (fonction désactivée).
	 */
	...
	public static function build( $key, $args ) {
		...
		foreach ( $args as $arg ) {
			...
			if ( 'file' === $arg['type'] || 'target' === $arg['type'] ) {
				$piece = self::validate_path( $arg['value'], $arg['type'] );
				if ( false === $piece ) {
					$built['reason'] = ( 'file' === $arg['type'] ) ? 'fichier' : 'cible';
					return $built;
				}
				$pieces[] = $piece;
				continue;
	...
	/**
	 * Exécute via la passerelle — unique point exec() du code livré.
	 */
	public static function run( $key, $args = array() ) {
		...
		$code   = 1;
		$t0     = microtime( true );
		@exec( $built['command'], $output, $code ); // nosemgrep: php.lang.security.exec-use.exec-use -- point d'appel unique cloisonné (SECU-1) : binaire en liste blanche (chemin absolu + empreinte), arguments fichier/cible protégés par escapeshellarg, drapeaux validés sans métacaractère
		$ms     = (int) round( ( microtime( true ) - $t0 ) * 1000 );
		$out['exit_code']   = $code;
		$out['output']      = is_array( $output ) ? $output : array();
		$out['duration_ms'] = $ms;
		$out['ok']          = ( 0 === $code );
		if ( ! $out['ok'] ) {
			$out['reason'] = 'exit-' . $code;
		}
	 ...
	 * existence + confinement realpath (entrée), répertoire parent confiné (cible).
	 */
	private static function validate_path( $value, $kind ) {
